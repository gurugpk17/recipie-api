...
export default function Search() {
  ...
  const [apiResult, apiSetResult] = React.useState([]);
  ...
  return (
    <div >
      <form className="search" onSubmit={handleSubmit}>
        <input
          id="search" type="search" value={search} onChange={onChangeHandler} placeholder="Search here..." required />
      ...
      {apiResult.map((item,index) => {
        return (
          <div key={index} className="dishes">
            <li className="dish-title" id="dish-title" onClick={() => handleClick(index)}>{item.title}
            </li>
            <div className="dish-desc">{showDetails[index] && (
              <div>
